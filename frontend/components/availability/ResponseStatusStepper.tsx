...
export default function ResponseStatusStepper({ level, count, nextExitDate, compact = false }: Props) {
  const activeColor = level >= 1 ? COLOR[level] : 'text-muted';
    ...
    <div className={compact ? 'space-y-3' : 'space-y-5'}>
      <div className="flex items-baseline justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted">No-respuestas en ventana: <span className="font-bold text-foreground">{count}</span></p>
        </div>
      </div>
              ...
              <div className="flex flex-col items-center gap-1.5 text-center">
                ...
                {!compact && (
                  <div className="leading-tight">
                    <p className={`text-xs font-bold ${isActive ? COLOR[node.n] : 'text-muted'}`}>{node.title}</p>
                    <p className="text-xs text-muted">{node.sub}</p>
                    <p className="text-xs text-muted">{node.range}</p>
                  </div>
                )}
              </div>
