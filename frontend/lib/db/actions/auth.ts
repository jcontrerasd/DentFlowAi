...
export async function requestEmailVerificationAction(email: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    const [recent] = await db
      ...
      .limit(1);
    if (recent && recent.expires.getTime() - Date.now() > (getVerificationTtlHours() * 3600_000 - VERIFICATION_COOLDOWN_MS)) {
      return { success: true };
    }
    ...
    const expires = new Date(Date.now() + getVerificationTtlHours() * 3600_000);
